...
#define PORT 13490  // the port users will be connecting to
...
void send_date(int fd) {
    // TODO
    //ctime() takes in time constant and spits out a null-terminated string.
    time_t now;
    char *date = ctime(&now);

    //concatinating
    char *date_front = "<h1>";
    strcat(date_front, date);
    char *date_end = "</h1>";
    strcat(date_front, date_end);
...
void send_hello(int fd, char *req) {
    ...
    if (req == NULL) {
        ...
        //can't do it need to concationate
        char *bmessage_front = "<h1>Hello, ";
        strcat(bmessage_front, req);
        char *bmessage = "!</h1>";
        strcat(bmessage, bmessage_front);
...
void handle_http_request(int fd) {
   ...
   // TODO
    ...
    // getting hello
    // CASE 1: hello
    else if (strncmp(req_path, "/hello", 6) == 0) { //if it has the same 6 characters
    // if it is only just hello, then the string is just NULL.
    send_hello(fd, NULL);
    }

    //CASE 2: hello, name
    else if (strncmp(req_path, "/hello?", 7) == 0) {
        char *name = NULL;
        //split the req_path into hello and name
        //store the string of name from path into name
        sscanf(req_path, "/hello?%s", name);
        send_hello(fd, name);
    }
