...
main_PTRs_init:
    lw  $t2, 4($a1)n                     # Load argv[1] into $t2
...
if_char_loop_row:
    lw  $t0, CHRSIZE
    bge $s3, $t0, end_if_char_loop_row
    li  $s4, 0                          # col = 0;

if_char_loop_col:
    lw  $t0, CHRSIZE
    bge $s4, $t0, end_if_char_loop_col
...
else_cond_upper:
    ...
    jal  isUpper                        # isUpper Uses: $a0, $v0                      # store its return value
    beqz  $v0, call_is_lower            # if (isUpper == 0)
    li   $t0, 'A'                       # which 'A'
    li   $t1, -1                        # var = -1
    mul  $t0, $t0, $t1                  # which = - 'A'
    add  $t0, $t0, $s2                  # which = ch - 'A'
    move $a0, $t0                       # which is in $a0
    j    found_which
...
else_char_loop_col:
    ...
    add $t2, $s4, $t2                   # offset = offset + col
...
end_outer_char_loop:
    ...
    addi $s4, $t0, -1                   # starting_col = NDCOLS - 1
    add  $s6, $t0, $s1                  # interations = NDCOLS + biglength
    									# Stored in $a_ because used as arguments later

    li   $s5, 0                         # i = 0;

final_loop:
    bge  $s5, $s6, end_final_loop		# if(i >= iterations)
    ...
    addi $s4, $s4, -1                   # starting_col--;
    addi $s5, $s5, 1

    li   $a0, 1                         # give argument of 1
    #jal  delay 							# delay uses: $a0
...
if_starting:
    ...
    li   $t0, -1                        # var0 = -1
    mul  $s3, $t0, $a0                  # first_col = -starting
    j    last_loop
...
copy_loop_in_col:
    ...
    lw  $t0, NDCOLS
    bge $s1, $t0, end_copy_loop_in_col  # if (out_col >= NDCOLS) break;
    li  $s0, 0                          # row = 0;

copy_loop_row:
    lw  $t0, NROWS
    bge $s0, $t0, end_copy_loop_row
...
inner_loop:
    ...
    lb   $a0, ($t2)                     # Load value of new_address
